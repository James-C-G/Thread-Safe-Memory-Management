 ...
 *  Version :           1.4
 *  Description :       Part 2 improves upon part 1 by implementing 4 methods of finding a node that can be used:
 *                      first fit (same as part 1), next fit, best fit, and worst fit. These algorithms all have
 *                      different methods for search over the memory manager for the best node to be used. This file
 *                      also implements a circular queue as we found it to be beneficial to be used in this part of the
 *                      project as it makes the algorithms easier to implement.
 *
 */
...
void *firstFit(size_t bytes)
{
    ...
    do
    {
        ...
        return ((void *)(freeNode(node, bytes)) + sizeof(Node));
 ...
 * node has been found, its details are changed and a new free node is created if a hole is created.
 *
 * @param bytes - requested bytes to be allocated
 * @return - void memory pointer/NULL if can't be allocated
 */
...
void *bestFit(size_t bytes)
{
    ...
    if (bestNode != NULL)
    {
        ...
        return ((void *)(bestNode) + sizeof(Node)); // End of loop met
    }
    return NULL;
}
...
void *worstFit(size_t bytes)
{
    ...
    if (worstNode != NULL)
    {
        ...
        return ((void *)(worstNode) + sizeof(Node)); // End of loop met
    }
    return NULL;
}
